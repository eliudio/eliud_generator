...
class GenerateSpecification {
  ...
  final bool
      hasPersistentRepository; // not generating a firestore repository doesn't mean there is no repository. Generate a repository doesn't mean there's a persistent repository. We need to know if there is a persistent repository in some cases, such as when wanting to wipe the repository
  ...
  final bool
      generateEmbeddedComponent; // is this an embedded internal component?
  final bool
      isExtension; // is this an extension, is this a component that can be added to a page
  final String?
      documentSubCollectionOf; // is this a subcollection and if so of which doc?

  bool isAppSubCollection() =>
      documentSubCollectionOf != null &&
      documentSubCollectionOf!.toLowerCase() == "app";

  GenerateSpecification(
      {required this.generateComponent,
      required this.generateRepository,
      required this.generateCache,
      required this.hasPersistentRepository,
      required this.generateFirestoreRepository,
      required this.generateRepositorySingleton,
      required this.generateModel,
      required this.generateEntity,
      required this.generateForm,
      required this.generateList,
      required this.generateDropDownButton,
      required this.generateInternalComponent,
      required this.generateEmbeddedComponent,
      required this.isExtension,
      required this.documentSubCollectionOf});
  ...
  List<Object> get props => [
        ...
        generateRepository,
        generateCache,
        generateFirestoreRepository,
        generateRepositorySingleton,
        hasPersistentRepository,
        generateModel,
        generateEntity,
        generateForm,
        generateList,
  ...
  static GenerateSpecification fromJson(Map<String, dynamic> json) {
    return GenerateSpecification(
        ...
        generateFirestoreRepository:
            json["generateFirestoreRepository"] as bool? ?? false,
        generateRepositorySingleton:
            json["generateRepositorySingleton"] as bool? ?? false,
        hasPersistentRepository:
            json["hasPersistentRepository"] as bool? ?? false,
        ...
        generateDropDownButton:
            json["generateDropDownButton"] as bool? ?? false,
        generateInternalComponent:
            json["generateInternalComponent"] as bool? ?? false,
        generateEmbeddedComponent:
            json["generateEmbeddedComponent"] as bool? ?? false,
