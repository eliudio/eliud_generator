...
class EntityCodeGenerator extends DataCodeGenerator {
  EntityCodeGenerator({required super.modelSpecifications});
  ...
  String _copyWith() {
    ...
    codeBuffer.writeln("}) {");
    codeBuffer
        .write("${spaces(4)}return ${modelSpecifications.entityClassName()}(");
    for (var field in modelSpecifications.fields) {
      if (field.fieldName != "documentID") {
        codeBuffer.write(
            "${fieldName(field)} : ${fieldName(field)} ?? this.${fieldName(field)}, ");
      }
    }
    codeBuffer.writeln(");");
...
*/
  ...
  String commonImports() {
    ...
    headerBuffer
        .writeln("import 'package:eliud_core_helpers/base/entity_base.dart';");
  ...
  String _fromMap() {
    StringBuffer codeBuffer = StringBuffer();
    codeBuffer.writeln(
        "${spaces(2)}static ${modelSpecifications.entityClassName()}? fromMap(Object? o, {Map<String, String>? newDocumentIds}) {");
    ...
    for (var field in modelSpecifications.fields) {
      if (field.isMedium() && !field.isArray()) {
        codeBuffer.writeln(
            "${spaces(4)}var ${fieldName(field)}NewDocmentId = map['${fieldName(field)}'];");

        codeBuffer.writeln(
            "${spaces(4)}if ((newDocumentIds != null) && (${fieldName(field)}NewDocmentId != null)) {");
        codeBuffer.writeln(
            "${spaces(6)}var ${fieldName(field)}OldDocmentId = ${fieldName(field)}NewDocmentId;");
        codeBuffer.writeln(
            "${spaces(6)}${fieldName(field)}NewDocmentId = newRandomKey();");
        codeBuffer.writeln(
            "${spaces(6)}newDocumentIds[${fieldName(field)}OldDocmentId] = ${fieldName(field)}NewDocmentId;");
        codeBuffer.writeln("${spaces(4)}}");
      } else if (field.isBespoke()) {
        codeBuffer.writeln(
            "${spaces(4)}var ${fieldName(field)}FromMap = map['${fieldName(field)}'];");
        codeBuffer
            .writeln("${spaces(4)}if (${fieldName(field)}FromMap != null)");
        codeBuffer.writeln(
            "${spaces(6)}${fieldName(field)}FromMap = ${field.getBespokeEntityMapping()};");
      } else {
        if (field.arrayType != ArrayType.CollectionArrayType) {
          if ((!field.isEnum()) &&
            ...
            if (field.isArray()) {
              if (field.arrayType != ArrayType.CollectionArrayType) {
                codeBuffer.writeln(
                    "${spaces(4)}var ${fieldName(field)}FromMap = map['${fieldName(field)}'];");
                codeBuffer.writeln(
                    "${spaces(4)}List<${field.fieldType}Entity> ${fieldName(field)}List;");
                codeBuffer.writeln(
                    "${spaces(4)}if (${fieldName(field)}FromMap != null) {");
                codeBuffer.writeln(
                    "${spaces(6)}${fieldName(field)}List = (map['${fieldName(field)}'] as List<dynamic>)");
                codeBuffer.writeln("${spaces(8)}.map((dynamic item) =>");
                codeBuffer.writeln(
                    "${spaces(8)}${field.fieldType}Entity.fromMap(item as Map, newDocumentIds: newDocumentIds)!)");
                codeBuffer.writeln("${spaces(8)}.toList();");
                codeBuffer.writeln("${spaces(4)}} else {");
                codeBuffer.writeln("${spaces(6)}${fieldName(field)}List = [];");
                codeBuffer.writeln("${spaces(4)}}");
              } else {
              ...
              codeBuffer.writeln(
                  "${spaces(4)}if (${fieldName(field)}FromMap != null)");
              codeBuffer.writeln(
                  "${spaces(6)}${fieldName(field)}FromMap = ${field.fieldType}Entity.fromMap(${fieldName(field)}FromMap, newDocumentIds: newDocumentIds);");
            }
          } else if (field.isMap()) {
            codeBuffer
                .writeln(process(_convertToMap, parameters: <String, String>{
          ...
          if (field.isServerTimestamp()) {
            if (field.isServerTimestampInitialized()) {
              ...
              var documentSubCollectionOf =
                  modelSpecifications.generate.documentSubCollectionOf;
              if (documentSubCollectionOf != null) {
                if (modelSpecifications.generate.isAppSubCollection()) {
                  ...
                  repoArgs =
                      "appId: map['appId'], ${lowerCase}Id: map['${lowerCase}Id']";
                }
              } else {
                repoArgs = '';
              }
              codeBuffer.writeln(
                  "map['${fieldName(field)}'] == null ? null : (map['${fieldName(field)}']  as Timestamp).millisecondsSinceEpoch,");
            } else {
            ...
            if (!field.isNativeType()) {
              ...
              if (field.isArray()) {
                codeBuffer.writeln(
                    "map['${fieldName(field)}'] == null ? null : List.from(map['${fieldName(field)}']), ");
              } else {
                if (field.isDouble()) {
                  codeBuffer.writeln(
                      "double.tryParse(map['${fieldName(field)}'].toString()), ");
                } else if (field.isInt())
                  codeBuffer.writeln(
                      "int.tryParse(map['${fieldName(field)}'].toString()), ");
                else
                  codeBuffer.writeln("map['${fieldName(field)}'], ");
              }
            }
  ...
  String _toDocument() {
        ...
        if (field.arrayType != ArrayType.CollectionArrayType) {
          if ((!field.isEnum()) &&
            ...
            if (field.isArray()) {
              if (field.arrayType != ArrayType.CollectionArrayType) {
                codeBuffer.writeln(
                    "${spaces(4)}final List<Map<String?, dynamic>>? ${fieldName(field)}ListMap = ${fieldName(field)} != null ");
                codeBuffer.writeln(
                    "${spaces(8)}? ${fieldName(field)}!.map((item) => item.toDocument()).toList()");
              ...
              codeBuffer.writeln(
                  "${spaces(4)}final Map<String, dynamic>? ${fieldName(field)}Map = ${fieldName(field)} != null ");
              codeBuffer
    ...
    for (var field in modelSpecifications.fields) {
      if (field.isServerTimestamp()) {
        codeBuffer.writeln(
            '${spaces(4)}theDocument["${fieldName(field)}"] = ${fieldName(field)};');
      } else if (field.isBespoke()) {
        ...
        if (field.arrayType != ArrayType.CollectionArrayType) {
          if (field.fieldName != "documentID") {
            codeBuffer.write(
                "${spaces(4)}if (${fieldName(field)} != null) theDocument[\"${fieldName(field)}\"] = ");
            ...
            codeBuffer.writeln(
                "${spaces(6)}else theDocument[\"${fieldName(field)}\"] = null;");
          }
        }
      }
    }
  ...
  String _switchAppId() {
    ...
    if (modelSpecifications.fields
        .where((element) => element.fieldName == "appId")
        .isNotEmpty) {
      codeBuffer
          .writeln("${spaces(4)}var newEntity = copyWith(appId: newAppId);");
    } else {
